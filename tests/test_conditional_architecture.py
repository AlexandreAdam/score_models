...
def test_continuous_timelike_conditional():
    nf = 32
    net = NCSNpp(nf=nf, ch_mult=(1, 1), condition=["continuous_timelike"])
...
def test_continuous_input_conditional():
    nf = 32
    C_cond = 3
    net = NCSNpp(nf=nf, ch_mult=(1, 1), condition=["input"], condition_input_channels=3)
...
def test_vector_condition():
    nf = 32
    C_cond = 3
    net = NCSNpp(nf=nf, ch_mult=(1, 1), condition=["vector"], condition_vector_channels=3)
...
def test_conditional_architecture_raising_errors():
    ...
    with pytest.raises(ValueError):
        net = NCSNpp(
            nf=nf, ch_mult=(1, 1), condition=["discrete_timelike"], condition_num_embedding=15
        )
