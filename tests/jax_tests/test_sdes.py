...
def get_trajectories(sde, key, B=10, N=100, x0=5):
    dt = 1/N
    t = jnp.zeros(B) + sde.epsilon
    x0 = jnp.ones(B) * x0
    x = x0.copy()
    trajectories = [x]
    marginal_samples = [x]
...
if __name__ == "__main__":
    ...
    for i, sde in enumerate([sde1, sde2, sde3, sde4, sde5]):
        trajectories, marginal_samples = get_trajectories(sde, random.fold_in(key, i), B, N, x0=x0)
        ...
        mu, sigma = sde.marginal_prob_scalars(jnp.array(t))
