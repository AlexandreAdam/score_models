...
def upfirdn3d_native(
    ...
    out = out.reshape(-1, in_h * up_y, in_w * up_x, in_d * up_z, 1)

    # Pad with zeros at the boudaries.
    out = jnp.pad(
        out,
        [
            (0, 0),
    ...
    # Reshape for convolution (NHWDI -> NIHWD)
    ...
    # Flip spatial + reshape kernel for convolution (HWD -> OIHWD)
    w = jnp.flip(kernel, [0, 1]).reshape(1, 1, kernel_h, kernel_w, kernel_d)
    out = lax.conv(out, w, window_strides=(1, 1, 1), padding="VALID")
    
    # Permute back to NHWDI
    out = out.reshape(
        -1,
        1,
