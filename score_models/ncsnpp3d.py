...
from .layers import DDPMResnetBlock, GaussianFourierProjection, SelfAttentionBlock, Conv3dSame
from .layers.resnet_block_biggan3d import ResnetBlockBigGANpp3d, upsample_3d, Conv3d, downsample_3d
...
def conv3x3x3(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1.):
    """3x3x3 convolution with DDPM initialization."""
...
class DownsampleLayer(nn.Module):
    def __init__(self, in_ch=None, out_ch=None, with_conv=False, fir=False,
        ...
        if not fir:
            if with_conv:
                self.Conv_0 = conv3x3x3(in_ch, out_ch, stride=2)
        else:
            if with_conv:
                self.Conv3d_0 = Conv3d(in_ch, out_ch,
                                        kernel=3, down=True,
...
class UpsampleLayer(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W, D = x.shape
        if not self.fir:
            h = F.interpolate(x, (H * 2, W * 2, D * 2), 'nearest')
            if self.with_conv:
                h = self.Conv_0(h)
        else:
            if not self.with_conv:
                h = upsample_3d(x, self.fir_kernel, factor=2)
            else:
                h = self.Conv3d_0(x)
        return h
...
class Combine(nn.Module):
    ...
    def __init__(self, dim1, dim2, method='cat'):
        super().__init__()
        self.Conv_0 = conv1x1x1(dim1, dim2)
        self.method = method
...
class NCSNpp3d(nn.Module):
    ...
    def __init__(
        ...
        AttnBlock = functools.partial(SelfAttentionBlock, init_scale=init_scale, d=3)
        ...
        if resblock_type == 'ddpm':
            ...
            ResnetBlock = functools.partial(ResnetBlockBigGANpp3d,
                                            act=act,
                                            dropout=dropout,
                                            fir=fir,
                                            fir_kernel=fir_kernel,
        ...
        modules.append(conv3x3x3(channels, nf))
        hs_c = [nf]
            ...
            if progressive != 'none':
                if i_level == num_resolutions - 1:
                    if progressive == 'output_skip':
                        ...
                        modules.append(conv3x3x3(in_ch, channels, init_scale=init_scale))
                        pyramid_ch = channels
                    elif progressive == 'residual':
                        modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                                    num_channels=in_ch, eps=1e-6))
                        modules.append(conv3x3x3(in_ch, in_ch, bias=True))
                        pyramid_ch = in_ch
                    else:
                    ...
                    if progressive == 'output_skip':
                        modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                                    num_channels=in_ch, eps=1e-6))
                        modules.append(conv3x3x3(in_ch, channels, bias=True, init_scale=init_scale))
                        pyramid_ch = channels
                    elif progressive == 'residual':
        ...
        if progressive != 'output_skip':
            modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                        num_channels=in_ch, eps=1e-6))
            modules.append(conv3x3x3(in_ch, channels, init_scale=1.))
