...
class ScoreModelBase(Module, ABC):
    def __init__(
        ...
        **hyperparameters,
        ...
        if model is None or isinstance(model, str):
            model, hyperparams, self.loaded_checkpoint = (
                load_architecture(  # TODO move this in a method we can overwrite for LORA w
        ...
        if sde is None:
            ...
            if "t_star" in hyperparameters.keys():
                ...
                raise KeyError(
                    "SDE parameters are missing, please specify which sde to use"
                )
        ...
        for _ in tqdm(range(ode_steps), disable=disable):
            if method == "Euler":
                ...
                raise NotImplementedError(
                    "Invalid method, please select either Euler or Heun"
                )
    ...
    def sample(
        ...
        steps,
        condition: list = [],  # TODO add the option to specify a time other than t=1, conditioned on some x0, maybe add a method called sample_around
        ...
        for _ in (pbar := tqdm(range(steps))):
            ...
            f = self.sde.drift(t, x) - g**2 * (
                self.score(t, x, *condition)
                + guidance_factor * likelihood_score_fn(t, x)
            )
    ...
    def fit(
        ...
        if checkpoints_directory is not None or logdir is not None:
            ...
            script_params_path = os.path.join(
                checkpoints_directory, "script_params.json"
            )
            ...
            model_hparams_path = os.path.join(
                checkpoints_directory, "model_hparams.json"
            )
            ...
            scores = [
                float(
                    re.findall(
                        "([0-9]{1}.[0-9]+e[+-][0-9]{2})", os.path.split(path)[-1]
                    )[-1]
                )
                for path in paths
            ]
            if checkpoint_indices:
                if model_checkpoint is not None:
                    ...
                    optimizer.load_state_dict(
                        torch.load(
                            opt_paths[checkpoints == model_checkpoint],
        ...
        for epoch in (pbar := tqdm(range(epochs))):
            ...
            ) > max_time * 3600 - estimated_time_for_epoch:
                break
            ...
            for _ in range(n_iterations_in_epoch):
                ...
                if clip > 0:
                    torch.nn.utils.clip_grad_norm_(
                        self.model.parameters(), max_norm=clip
                    )
            ...
            if save_checkpoint:
                if (
                    ...
                    checkpoint_indices = [
                        int(re.findall("[0-9]+", os.path.split(path)[-1])[-1])
                        for path in paths
                    ]
                    scores = [
                        float(
